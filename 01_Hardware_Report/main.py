"""
Generates a PDF file report containing information about the computer's hardware,
analyzing it within a user defined time period.
...
stop_time = datetime(start_time.year,
                     start_time.month,
                     start_time.day,
                     start_time.hour,
                     start_time.minute+1,  # The stop_time increases 1 minute from the start_time
                     start_time.second,
...
class HardwareReport:
    ...
    with numerical and graphical information of the System, CPU, Memory, Disks and Network.
    """
    ...
    def __init__(self,
        ...
        :output
            pdf file with the computer's hardware report
        """
        ...
        # This variable "parts" will append all parts (text, images, tables) for the PDF
        self.parts = []
        self.parts.append(PdfBuilder.logo)

        # Header
        text = f"""
        COMPUTER'S HARDWARE REPORT
        ...
        while start_time.timestamp() < current_time.timestamp() < stop_time.timestamp():
            ...
            if cpu:
                ...
                # Append a float with the current system-wide CPU utilization as a percentage
            ...
            if memory:
                ...
                self.memory_data["Total"].append(
                    float(self.convert_bytes_to_readable_measurement(self.svmem.total)[:-2]))
                # Append the total swap memory in bytes
                self.swap_data["Total"].append(
                    float(self.convert_bytes_to_readable_measurement(self.swap.total)[:-2]))
                # Append the memory (in bytes) to processes without the system going into swap
                self.memory_data["Available"] = float(
                    self.convert_bytes_to_readable_measurement(self.svmem.available)[:-2])
                ...
                self.swap_data["Free"] = float(
                    self.convert_bytes_to_readable_measurement(self.swap.free)[:-2])
                # Append memory used, calculated differently depending on the platform and
                # designed for informational purposes only
                self.memory_data["Used"].append(
                    float(self.convert_bytes_to_readable_measurement(self.svmem.used)[:-2]))
                # Append used swap memory in bytes
                self.swap_data["Used"].append(
                    float(self.convert_bytes_to_readable_measurement(self.swap.used)[:-2]))

                # Append the percentage usage calculated as (total - available) / total * 100
                self.memory_data["Percentage_usage"].append(self.svmem.percent)
                # Append the percentage usage calculated as (total - free) / total * 100
            ...
            current_time = datetime.now()  # Updates the current time
    ...
    def generate_pdf(self):
        """
        Main method containing all functions to generate the PDF report
        """        
        if self.system:
            """
        ...
        if self.network:
            ...
            # Return information about each NIC (network interface card) installed on the system
            ...
            for nic, addrs in psutil.net_if_addrs().items():
                ...
                if nic in stats:
                    st = stats[nic]
                    data.append(
                        ["", "stats", f": speed={st.speed}MB, duplex={duplex_map[st.duplex]}, mtu={st.mtu}, up={'yes' if st.isup else 'no'}"])
                if nic in io_counters:
                    io = io_counters[nic]
                    data.append(
                        ["", "incoming", f": bytes={bytes2human(io.bytes_recv)}, pkts={io.packets_recv}, errs={io.errin}, drops={io.dropin}"])
                    data.append(
                        ["", "outgoing", f": bytes={bytes2human(io.bytes_sent)}, pkts={io.packets_sent}, errs={io.errout}, drops={io.dropout}"])
                for addr in addrs:
                    data.append(
                        ["", f"{af_map.get(addr.family, addr.family):4} address", f": {addr.address}"])
                    if addr.broadcast:
                        data.append(
